// Implementation of a TLO_Agent_MOSSTP which uses an augmented state based on accumulated expected rewards 
...
// It also divide into two phrase data-gathering and learning phase
...
public class TLO_Agent_MOSSTP implements AgentInterface 
{
    ...
    int episodeCounter; //count of all episodes v_\pi
    ...
    private int updateStatisticsHelper(int envStateS, double[] accumulatedRewardsP){
        ...
        //Lower bound on probability weighting
        double lowerBoundProbability = 0.0001;
        // estimated probability of visiting s in any episode
        double estimatedProbability = Math.max((double)stateVisits[envStateS]/episodeCounter,lowerBoundProbability);
        // estimated return in episodes where s is not visited
        double estimatedReturnNotVisited[] = getEstimatedReturnNotVisited(estimatedProbability, envStateS);
    ...
    private int getLearningPhaseAugmentedStateIndex(int envStateS, double[] accumulatedRewardsP){
        double estimatedProbability = (double)stateVisits[envStateS]/episodeCounter;
        // estimated return in episodes where s is not visited
        double estimatedReturnNotVisited[] = getEstimatedReturnNotVisited(estimatedProbability, envStateS);
    ...
    private double[] getEstimatedReturnNotVisited(double estimatedProbability, int envStateS){
        double estimatedReturnNotVisited[] = new double [numThresholds];
        for (int i=0; i<numThresholds; i++){
        ...
        return estimatedReturnNotVisited;
    }
    ...
    public Action agent_start(Observation observation) {
        ...
        if(learningPhaseFlag){
            state = getLearningPhaseAugmentedStateIndex(envState, expectedAccumlatedRewards[envState]);
            action = getAction(state);
        }else{
            state = updateStatisticsHelper(envState, accumulatedRewards);
            action = ((ActionSelector)vf).chooseGreedyAction(state);
        }
    ...
    public Action agent_step(Reward reward, Observation observation) 
    {
        ...
        if(learningPhaseFlag){
            state = getLearningPhaseAugmentedStateIndex(lastState, expectedAccumlatedRewards[lastState]);
            ...
            if (!policyFrozen) {
                ...
                state = updateStatisticsHelper(lastState, accumulatedRewards);
            }else{
                state = getLearningPhaseAugmentedStateIndex(lastState, expectedAccumlatedRewards[lastState]);
            }
            action = ((ActionSelector)vf).chooseGreedyAction(state);
        ...
        if(learningPhaseFlag){
            ...
            if (!policyFrozen) {
                ...
                for(int o=0; o<numThresholds; o++){
                    estimatedOverallReturn[o] += ((double)1/episodeCounter)*(accumulatedRewards[o]-estimatedOverallReturn[o]);
                    for(int s=0; s<numEnvtStates; s++){
                        if(stateFlag[s]!=0){
    ...
    public String agent_message(String message) {
        ...
        if (message.startsWith("change_weights")){
            System.out.print("TLO_Agent_MOSSTP: Weights can not be changed");
            return "TLO_Agent_MOSSTP: Weights can not be changed";
        }
        ...
        if (message.startsWith("set_learning_parameters")){
        	...
        	System.out.println("Alpha = " + startingAlpha + " Lambda = " + lambda + " Gamma = " + gamma + " exploration = " + TLO_LookupTable.explorationStrategyToString(explorationStrategy) + " alpha decay = " +alphaDecay + " Data-gathering episode = " + parts[6] + " Learning episode = "+ parts[7]);
        ...
        if (message.startsWith("print_lookup_table")){
            ...
            for(int s=0; s<numEnvtStates; s++){
                ...
                for(int a=0; a<numActions; a++){
                    ...
                    {
                        double lowerBoundProbability = 0.0001;
                        double probability = Math.max((double)stateVisits[s]/episodeCounter,lowerBoundProbability);
                        ...
                        System.out.print("estimatedOverallReturn: "+estimatedOverallReturn[i]+" ");
                        System.out.print("estimatedOverallStatedReturn: "+estimatedOverallStatedReturn[s][i]+" ");
        ...
        if(message.equals("print_data")){
            
            int policy = 0;
            double lowerBoundProbability = 0.0001;
            double probability = Math.max((double)stateVisits[0]/episodeCounter,lowerBoundProbability);

            int augmentedState1 = getAugmentedStateIndex(0, expectedAccumlatedRewards[0]);
            vf.setConditioningValues(expectedAccumlatedRewards[0], probability, getEstimatedReturnNotVisited(probability,0));
            policy = ((ActionSelector)vf).chooseGreedyAction(augmentedState1)*3;


            probability = Math.max((double)stateVisits[1]/episodeCounter,lowerBoundProbability);
            String data = String.valueOf(probability);

            for(int a=0; a<numActions; a++){
                double augmentedQValue = vf.getThisStateValues()[a][0];
                data += "&"+ String.valueOf(augmentedQValue);
            }

            int augmentedState2 = getAugmentedStateIndex(1, expectedAccumlatedRewards[1]);
            vf.setConditioningValues(expectedAccumlatedRewards[1], probability, getEstimatedReturnNotVisited(probability,1));
            policy += ((ActionSelector)vf).chooseGreedyAction(augmentedState2);

            for(int a=0; a<numActions; a++){
                double augmentedQValue = vf.getThisStateValues()[a][0];
                data += "&"+ String.valueOf(augmentedQValue);
            }
            data +="&"+String.valueOf(policy);
            return data;
        }
        ...
        if(message.equals("data_gathering_phase")){
            learningPhaseFlag = false;
            return "Turn on data_gathering_phase";
        }
        if(message.equals("learning_phase")){
            learningPhaseFlag = true;
            return "Turn on learning_phase";
        }
        ...
        System.out.println("TLO_Agent_MOSSTP - unknown message: " + message);
        return "TLO_Agent_MOSSTP does not understand your message.";
    }
    

    public static void main(String[] args) {
        AgentLoader theLoader = new AgentLoader( new TLO_Agent_MOSSTP() );
        theLoader.run();
    }
}
