// Implementation of a TLO_Agent which uses an augmented state based on accumulated actual rewards to get around issues caused by
// non-additivity of rewards in the Bellman equation.
...
public class TLO_Agent_Conditioned_On_Actual_Rewards implements AgentInterface 
{
    ...
    /*= 10; //10; // how many divisions in the discretisation of the accumulated reward?
    ...
    */
    ...
    public String agent_message(String message) {
        ...
        if (message.startsWith("change_weights")){
            System.out.print("TLO_Agent_Conditioned_On_Actual_Rewards: Weights can not be changed");
            return "TLO_Agent_Conditioned_On_Actual_Rewards: Weights can not be changed";
        }
        ...
        if (message.startsWith("set_TLO_parameters")){
        	...
        	for (int i=0; i<numThresholds; i++)
        	{
	        	...
	        	numDiscretisations[i] = (int)Double.valueOf(parts[2]).doubleValue();
        ...
        if (message.startsWith("set_softmax_parameters")){
        	...
        	temperatureDecayRatio = Math.pow(0.01/startingTemperature,1.0/numEpisodes);//Math.pow(0.01/startingTemperature,1.0/numEpisodes);
        ...
        System.out.println("TLO_Agent_Conditioned_On_Actual_Rewards - unknown message: " + message);
        return "TLO_Agent_Conditioned_On_Actual_Rewards does not understand your message.";
    }
    ...
    public void dumpInfo(Observation observation, Action thisAction, Action otherAgentAction)
    {
        ...
        System.out.println("TLO_Agent_Conditioned_On_Actual_Rewards");
    ...
    public static void main(String[] args) {
        AgentLoader theLoader = new AgentLoader( new TLO_Agent_Conditioned_On_Actual_Rewards() );
        theLoader.run();
