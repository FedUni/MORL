...
// Implements a variant of the Deep Sea Treasure MORL benchmark problem. The original DST has a concave front, where only
// the two extreme policies can be found by linear scalarisation. This variant changes the values of some treasures, so that two 
// additional policies are now on the convex hull - i.e. the Pareto front is now a mixture of concave and convex points.
package env;
...
public class DeepSeaTreasureMixed implements EnvironmentInterface
{  
	// define the structure of the environment, and the treasure rewards
    private int depths[] = {1, 2, 3, 4, 4, 4, 7, 7, 9, 10};
    private float treasure[] = {1, 2, 10, 11, 12, 13, 15, 18.5f, 19, 20};
    ...
    {
        EnvironmentLoader theLoader = new EnvironmentLoader(new DeepSeaTreasureMixed());
        theLoader.run();
    }
