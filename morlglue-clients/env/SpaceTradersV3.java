// Modified version of Space Trader with -1 Reward and Plant C in between Plant A and terminal state
package env;
...
public class SpaceTradersV3 implements EnvironmentInterface
{  
    ...
    public void updatePositionAndReward(int theAction) 
    {
    	if (currentState==0) // Planet A
    	{
    		...
    		else // death
    		{
                ...
                {
                    rewards.setDouble(0, 0); // no penalty or reward for mission success until ended
                    rewards.setDouble(1,-1); // use the success time penalty
                    currentState = 2;
                }
                else{
                    rewards.setDouble(0, 0); // Receive -1 penalty for failure
                    rewards.setDouble(1,STATE_A_DATA[theAction][2]); // use the failure time penalty
                    currentState = 3;
                    terminal = true;
                }
        	}
        ...
        else if (currentState==1)
        {
    		...
    		{
    			rewards.setDouble(0, -1); // Receive -1 penalty for failure
    			rewards.setDouble(1,STATE_B_DATA[theAction][2]); // use the failure time penalty
        	}
			currentState = 3;
			terminal = true;
        }
        ...
        {
            rewards.setDouble(0, -1); // Receive -1 penalty for failure
            rewards.setDouble(1, -1); // use the failure time penalty
            currentState = 3;
			terminal = true;
        }
    }   
    ...
    {
        EnvironmentLoader theLoader = new EnvironmentLoader(new SpaceTradersV3());
        theLoader.run();
    }
